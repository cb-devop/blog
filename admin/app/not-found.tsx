...
import { Shield, AlertTriangle, Home, ArrowLeft, Search, Terminal, ChevronRight } from "lucide-react";
...
export default function AdminNotFound() {
  ...
  return (
    <div className="relative min-h-[80vh] flex items-center justify-center p-4 overflow-hidden">
      ...
      {/* Floating Orbs */}
      <div className="absolute top-1/3 -left-32 w-80 h-80 bg-red-500/10 rounded-full blur-[128px] animate-pulse" />
          ...
          <div className="p-6 md:p-8 space-y-6">
            ...
            <div className="text-center">
              ...
              <h1 className="text-xl font-bold text-foreground mt-4 font-mono">
                <span className="text-red-500">$</span> Route Not Found
              </h1>
              <p className="text-muted-foreground mt-2 font-mono text-sm">
                The admin panel page you requested does not exist
              </p>
            </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="space-y-1.5 text-xs">
                  <div>
                    ...
                    <span className="text-blue-500">route</span> --resolve{" "}
                    <span className="text-amber-500">{typeof window !== "undefined" ? window.location.pathname : "/unknown"}</span>
                  </div>
                  <div className="text-red-500">
                    <span className="text-red-500/70">❌</span> Status:{" "}
                    <span className="font-bold text-red-500">404 Not Found</span>
                  </div>
                  <div className="text-muted-foreground">
                    <span className="text-violet-500">[DIAGNOSTIC]</span> Scanning route table{dots}
                  </div>
                  <div className="text-muted-foreground">
                    <span className="text-red-500">[FAILED]</span> No matching entry in{" "}
                    <span className="text-blue-500">/etc/admin/routes</span>
                  </div>
                  {progress < 100 ? (
                    <div className="flex items-center gap-2 text-muted-foreground">
                      <span className="text-amber-500">[RECOVERY]</span> Attempting fallback...
                      <span className="text-primary font-semibold">{Math.round(progress)}%</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2 text-emerald-500">
                      <span className="text-emerald-500">[SUCCESS]</span> Redirecting to dashboard
                    </div>
                  )}
                </div>
              </div>
